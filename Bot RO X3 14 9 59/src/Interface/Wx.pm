...
sub createMenuBar {
	...
	$self->addMenu ($viewMenu, T('&ดูไอเท็มในตัว') . "\tAlt+E", sub { $self->openInventory (1) });
	$self->addMenu ($viewMenu, T('&ดูไอเท็มในรถ') . "\tAlt+W", sub { $self->openCart (1) });
	...
	$self->addMenu($viewMenu, T('&ดูสกิล'),		sub { Commands::run("skills"); });
	$viewMenu->AppendSeparator;
	...
	$menu->Append($viewMenu, T('&ดูข้อมูล'));
	...
	$settingsMenu->AppendSeparator;
	$self->{iDeal} = $self->addMenu($settingsMenu, 'ปิดเดล', sub {Commands::run("conf dealAuto 0")}, 'ปิดเดว');
	$self->{dDeal} = $self->addMenu($settingsMenu, 'ยกเลิืกการเดล', sub {Commands::run("conf dealAuto 1")}, 'ยกเลิืกการเดว');
	$self->{aDeal} = $self->addMenu($settingsMenu, 'เดล', sub {Commands::run("conf dealAuto 2")}, 'เดว');
	...
	$self->addMenu($dealMenu, 'ไม่แลกเปลี่ยน', sub {Commands::run("deal no")}, 'ไม่แลกเปลี่ยน');
	...
	$menu->Append($pgfMenu, 'ตี้/กิล/เพื่อน');
	...
	$self->addMenu($pgfMenu, 'เข้าตี้', sub {Commands::run("party join 1")}, 'เข้าติ็');
	...
	$self->addMenu($pgfMenu, 'ไม่เข้ากิล', sub {Commands::run("guild join 0")}, 'ไม่เข้ากิว');
	$pgfMenu->AppendSeparator;
	$self->addMenu($pgfMenu, 'ดูสมาชิกในตี้', sub {Commands::run("party")}, 'ดูสมาชิกในตี้');
	$self->addMenu($pgfMenu, 'ดูสมาชิกในกิล', sub {Commands::run("guild member")}, 'ดูสมาชิกในกิว');
	$self->addMenu($pgfMenu, 'ดูขัอมูลกิล', sub {Commands::run("guild info")}, 'ดูขัอมูลกิว');
	$self->addMenu($pgfMenu, 'ดูรายชื่อเพื่อน', sub {Commands::run("friend")}, 'ดูรายชื่อเพื่น');
