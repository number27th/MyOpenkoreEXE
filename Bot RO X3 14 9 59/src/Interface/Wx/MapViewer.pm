...
#
#  $Revision: 7534 $
#  $Id: MapViewer.pm 7534 2010-10-22 01:19:43Z kLabMouse $
#
#########################################################################
...
sub new {
	...
	$self->{textColor}{player}  = new Wx::Colour (0, 255, 255);
	$self->{brush}{monster}     = new Wx::Brush(new Wx::Colour(215, 0, 0), wxSOLID);
	$self->{textColor}{monster} = new Wx::Colour (127, 0, 0);
	$self->{brush}{item}     = new Wx::Brush(new Wx::Colour(0, 0, 200), wxSOLID);
	$self->{textColor}{item} = new Wx::Colour (0, 0, 255);
...
sub _loadMapImage {
	...
	if (-f $self->_map("$name.jpg")) {
		...
		return _loadImage($self->_map("$name.gif"), $self->{zoom});
...
#					) {
						...
						$self->_drawArrow($dc, $x, $y, $dest_x, $dest_y, 1);
#					}
				}
			}
			$dc->SetPen(wxBLACK_PEN);
		...
		foreach my $pos (@{$self->{portals}->{$self->{field}{name}}}) {
			...
			if ($self->{zoom} >= ($config{wx_map_namesDetail} || 1)) {
				$self->_drawText (
					$dc,
	...
	if ($self->{players} && @{$self->{players}}) {
		...
		foreach my $pos (@{$self->{players}}) {
			...
			if ($self->{zoom} >= ($config{wx_map_playerNameZoom} || 2)) {
	...
	if ($self->{monsters} && @{$self->{monsters}}) {
		...
		foreach my $pos (@{$self->{monsters}}) {
			...
			$self->_drawLook ($dc, $pos->{look}, $x, $y, $actor_r);
			$dc->DrawEllipse($x - $actor_r, $y - $actor_r, $actor_d, $actor_d);
			if ($self->{zoom} >= ($config{wx_map_namesDetail} || 3)) {
				$self->_drawText ($dc, $pos->name, $x, $y);
			}
		}
	}
	...
	$dc->SetTextForeground ($self->{textColor}{item});
	if ($self->{items} && @{$self->{items}}) {
		$dc->SetBrush($self->{brush}{item});
		foreach my $pos (@{$self->{items}}) {
			($x, $y) = $self->_posXYToView($pos->{pos_to}{x}, $pos->{pos_to}{y});
			$self->_drawLook ($dc, $pos->{look}, $x, $y, $actor_r);
			$dc->DrawEllipse($x - $actor_r, $y - $actor_r, $actor_d, $actor_d);
			if ($self->{zoom} >= ($config{wx_map_namesDetail} || 3)) {
