...
#
#  $Revision: 7534 $
#  $Id: Wx.pm 7534 2010-10-22 01:19:43Z kLabMouse $
#
#########################################################################
...
sub createMenuBar {
	...
	$self->{mPause}  = $self->addMenu($opMenu, T('&หยุด บอท'), \&onDisableAI, T('Pause all automated botting activity'));
	$self->{mManual} = $self->addMenu($opMenu, T('&เล่นเอง'), \&onManualAI, T('Pause automated botting and allow manual control'));
	$self->{mResume} = $self->addMenu($opMenu, T('&บอท ทำงาน'), \&onEnableAI, T('Resume all automated botting activity'));
	$opMenu->AppendSeparator;
	$self->addMenu($opMenu, T('&คัดลอกข้อความ  100 บรรทัด'), \&onCopyLastOutput);
	$self->addMenu($opMenu, T('&ล้างข้อความ'), sub {my $self = shift; $self->{console}->Remove(0, 40000)}, T('Clear content of console'));
	$self->addMenu($opMenu, '&ล้าง logs ข้อความ', sub {Commands::run("cl")}, ' &ล้าง logs Item');
	$self->addMenu($opMenu, '&ล้าง logs Item', sub {Commands::run("cil")}, ' &ล้าง logs Item');
	$self->addMenu($opMenu, T('&ซ่อนไว้ทางขวามือด้านล่าง'), \&onMinimizeToTray, T('Minimize to a small task bar tray icon'));
	$opMenu->AppendSeparator;
	$self->addMenu($opMenu, T('&กลับจุดเซฟ'), sub { Commands::run ("respawn") }, T('Teleport to save point'));
	$self->addMenu($opMenu, T('&บอททำงานใหม่ภายใน 5 วินาที'), sub { Commands::run ("relog") }, T('Disconnect and reconnect'));
	$self->addMenu($opMenu, T('&เลือกตัวละคร'), sub { Commands::run ("charselect");
	}, T('Exit to the character selection screen'));
	$self->addMenu($opMenu, T('ออก') . "\tCtrl-W", \&quit, T('Exit this program'));
	$menu->Append($opMenu, T('P&rogram'));

	# View menu
	my $viewMenu = $self->{viewMenu} = new Wx::Menu;
	$self->addMenu (
		$viewMenu, T('&ดูแผนที่') . "\tCtrl-M", \&onMapToggle, T('Show where you are on the current map')
	);
	$self->{infoBarToggle} = $self->addCheckMenu (
	$viewMenu, T('&Bar ข้อมุล'), \&onInfoBarToggle, T('Show or hide the information bar.'));
	$self->{chatLogToggle} = $self->addCheckMenu (
	$viewMenu, T('Chat &Log'), \&onChatLogToggle, T('Show or hide the chat log.'));
	$self->addMenu ($viewMenu, T('&ดูข้อมูุลตัวละคร') . "\tAlt+A", sub { $self->openStats (1) });
	...
	$self->addMenu ($viewMenu, T('&สัตว์เลี้ยง') . "\tAlt+J", sub { $self->openPet (1) });
	$viewMenu->AppendSeparator;
	$self->addMenu ($viewMenu, T('&ดูไอเ็ท็มในตัว') . "\tAlt+E", sub { $self->openInventory (1) });
	$self->addMenu ($viewMenu, T('&ดูไอเ้็ท็มในรถ') . "\tAlt+W", sub { $self->openCart (1) });
	$self->addMenu ($viewMenu, T('&เปิดคลัง'), sub { $self->openStorage (1) });
	$viewMenu->AppendSeparator;
	$self->addMenu ($viewMenu, T('&Emotions'). "\tAlt+L", sub { $self->openEmotions (1) });
	$viewMenu->AppendSeparator;
	$self->addMenu($viewMenu, T('&รายงานค่าประสบการณ์') . "\tCtrl+E", sub {
	$self->openWindow (T('รายงาน'), 'Interface::Wx::StatView::Exp', 1) });
	$viewMenu->AppendSeparator;
	$self->addMenu($viewMenu, T('&ดูข้อมูลตัวละคร').'	Alt-S',	sub { Commands::run("s"); });
	$self->addMenu($viewMenu, T('&ดูสถิติการทำงาน'),	sub { Commands::run("st"); });
	$self->addMenu($viewMenu, T('&ดูของในตัว').'	Alt-I',	sub { Commands::run("i"); });
	$self->addMenu($viewMenu, T('&ดูสกิว'),		sub { Commands::run("skills"); });
	$viewMenu->AppendSeparator;
	$self->addMenu($viewMenu, T('&ดูคนเล่น').'	Alt-P',	sub { Commands::run("pl"); });
	$self->addMenu($viewMenu, T('&ดูมอนเตอร์').'	Alt-M',	sub { Commands::run("ml"); });
	$self->addMenu($viewMenu, T('&ดู NPCs'),		sub { Commands::run("nl"); });
	$viewMenu->AppendSeparator;
	$self->addMenu($viewMenu, T('&รายงานค่าประสบการณ์'),	sub { Commands::run("exp"); });
	$self->addMenu($viewMenu, T('&รายงาน ไอเท็ม'),	sub { Commands::run("exp item"); });
	$self->addMenu($viewMenu, T('&รายงาน มอนเตอร์'),	sub { Commands::run("exp monster"); });
	$menu->Append($viewMenu, T('&ดูข้อมุล'));

	# Reload menu
	my $reloadMenu = new Wx::Menu;
	$self->addMenu($reloadMenu, T('&autotrade'),		sub { Commands::run("reload autotrade.txt");});
	$self->addMenu($reloadMenu, T('&cart_control'),		sub { Commands::run("reload cart_control.txt");});
	$self->addMenu($reloadMenu, T('&chat_resp'),		sub { Commands::run("reload chat_resp.txt");});
	$self->addMenu($reloadMenu, T('&config'),		sub { Commands::run("reload config.txt");});
	$self->addMenu($reloadMenu, T('&equip_control'),		sub { Commands::run("reload equip_control.txt");});
	$self->addMenu($reloadMenu, T('&intown_control'),		sub { Commands::run("reload intown_control.txt");});
	$self->addMenu($reloadMenu, T('&items_control'),		sub { Commands::run("reload items_control.txt");});
	$self->addMenu($reloadMenu, T('&mon_control'),		sub { Commands::run("reload mon_control.txt");});
	$self->addMenu($reloadMenu, T('&macros'),		sub { Commands::run("reload macros.txt");});
	$self->addMenu($reloadMenu, T('&pfroute'),		sub { Commands::run("reload pfroute.txt");});
	$self->addMenu($reloadMenu, T('&pickupitems'),		sub { Commands::run("reload pickupitems.txt");});
	$self->addMenu($reloadMenu, T('&plus_control'),		sub { Commands::run("reload plus_control.txt");});
	$self->addMenu($reloadMenu, T('&shop'),		sub { Commands::run("reload shop.txt");});
	$self->addMenu($reloadMenu, T('&skill_control'),		sub { Commands::run("reload skill_control.txt");});
	$self->addMenu($reloadMenu, T('&teleRoute_control'),		sub { Commands::run("reload teleRoute_control.txt"); });
	$self->addMenu($reloadMenu, T('&timeouts'),		sub { Commands::run("reload timeouts.txt");});
	$self->addMenu($reloadMenu, T('&all'),		sub { Commands::run("reload all");});
	$menu->Append($reloadMenu, T('&Reload'));
	
	# command menu
	my $commandMenu = new Wx::Menu;
	$menu->Append($commandMenu, T('&คำสั่ง'));
	$self->addMenu($commandMenu, T('&teleport'),		sub { Commands::run("tele"); });
	$self->addMenu($commandMenu, T('&นั้ง'),		sub { Commands::run("sit"); });
	$self->addMenu($commandMenu, T('&ยืน'),		sub { Commands::run("stand"); });
	$self->addMenu($commandMenu, T('&ที่ไหน'),		sub { Commands::run("where"); });
	$viewMenu->AppendSeparator;
	$self->addMenu($commandMenu, T('&ซื้อของ'),		sub { Commands::run("autobuy"); });
	$self->addMenu($commandMenu, T('&ขายของ'),		sub { Commands::run("autosell"); });
	$self->addMenu($commandMenu, T('&เก็บของ'),		sub { Commands::run("autostorage"); });
	$self->addMenu($commandMenu, T('&กลับจุดเซฟ'),		sub { Commands::run("respawn"); });
	$self->addMenu($commandMenu, '&เก็บของ  ซื้อของ ขายของ', sub {Commands::run("autostorage"); Commands::run("autosell"); Commands::run("autobuy")}, 'ทำทุกอย่าง');
	
	# Settings menu
	my $settingsMenu = new Wx::Menu;
	$self->createSettingsMenu($settingsMenu) if ($self->can('createSettingsMenu'));
	$self->addMenu ($settingsMenu, T('&ตั้งค่า Font'), \&onFontChange, T('Change console font'));
	$settingsMenu->AppendSeparator;
	$self->{iDeal} = $self->addMenu($settingsMenu, 'เปิดรับ กระซิปจากผู้เล่น', sub {Commands::run("conf ignoreAll 1")}, ' เปิดรับ กระซิปจากผู้เล่น');
	$self->{iDeal} = $self->addMenu($settingsMenu, 'ปิดรับ กระซิปจากผู้เล่น', sub {Commands::run("conf ignoreAll 0")}, 'เปิดรับ กระซิปจากผู้เล่น');
	$settingsMenu->AppendSeparator;
	$self->{iDeal} = $self->addMenu($settingsMenu, 'ปิด เดินแบบสุ่ม', sub {Commands::run("conf route_randomWalk 0")}, 'ปิด เดินแบบสุ่ม');
	$self->{iDeal} = $self->addMenu($settingsMenu, 'เดินแบบสุ่ม', sub {Commands::run("conf route_randomWalk 1")}, ' เดินแบบสุ่ม');
	$self->{iDeal} = $self->addMenu($settingsMenu, 'เดินแบบสุ่ม แต่ไม่เข้าจุดวาป', sub {Commands::run("conf route_randomWalk 2")}, ' เดินแบบสุ่ม แต่ไม่เข้าจุดวาป');
	$settingsMenu->AppendSeparator;
	$self->{iDeal} = $self->addMenu($settingsMenu, 'ไม่โจมตีมอน', sub {Commands::run("conf attackAuto 0")}, 'ไม่โจมตีมอน');
	$self->{iDeal} = $self->addMenu($settingsMenu, 'โจมตีอัติโนมัตื', sub {Commands::run("conf attackAuto 2")}, 'เปิดโจมตีอัติโนมัตื');
	$self->{iDeal} = $self->addMenu($settingsMenu, 'โจมตีอัติโนมัตื เมื่อโดนมอนตี', sub {Commands::run("conf attackAuto 1")}, 'เปิดโจมตีอัติโนมัตื เมื่อโดนมอนตี'); 
	$settingsMenu->AppendSeparator;
	$self->{iDeal} = $self->addMenu($settingsMenu, 'ฆ่ามอนสเตอร์ก่อน', sub {Commands::run("conf itemsTakeAuto 1")}, 'ฆ่ามอนสเตอร์ก่อน');
	$self->{iDeal} = $self->addMenu($settingsMenu, 'เก็บของก่อน ', sub {Commands::run("conf itemsTakeAuto 2")}, 'เก็บของก่อน');
	$self->{iDeal} = $self->addMenu($settingsMenu, 'ไม่เก็บของ', sub {Commands::run("conf itemsTakeAuto 0")}, 'ไม่เก็บของ');
	$settingsMenu->AppendSeparator;
	$self->{dParty} = $self->addMenu($settingsMenu, 'ไม่เข้าตี้', sub {Commands::run("conf partyAuto 1")}, 'ไม่เข้าตี้');
	$self->{aParty} = $self->addMenu($settingsMenu, 'เข้าตี้', sub {Commands::run("conf partyAuto 2")}, 'เข้าตี้');
	$self->{iParty} = $self->addMenu($settingsMenu, 'ไม่ตอบ', sub {Commands::run("conf partyAuto 0")}, 'ไม่ตอบ');
	$settingsMenu->AppendSeparator;
	$self->{iDeal} = $self->addMenu($settingsMenu, 'ปิดเดว', sub {Commands::run("conf dealAuto 0")}, 'ปิดเดว');
	$self->{dDeal} = $self->addMenu($settingsMenu, 'ยกเลิืกการเดว', sub {Commands::run("conf dealAuto 1")}, 'ยกเลิืกการเดว');
	$self->{aDeal} = $self->addMenu($settingsMenu, 'เดว', sub {Commands::run("conf dealAuto 2")}, 'เดว');
	$self->addMenu($settingsMenu, T('&ตั้งค่า Config.txt'), \&onAdvancedConfig, T('Edit advanced configuration options.'));
	$settingsMenu->AppendSeparator;
	$menu->Append($settingsMenu, T('&ตั้งค่า'));
	$self->createSettingsMenu2($settingsMenu) if ($self->can('createSettingsMenu2'));

	# deal
	my $dealMenu = new Wx::Menu();
	$menu->Append($dealMenu, 'แลกเปลี่ยน');
	$self->addMenu($dealMenu, 'แลกเปลี่ยน', sub {Commands::run("deal")}, 'แลกเปลี่ยน');
	$self->addMenu($dealMenu, 'ไมแลกเปลี่ยน', sub {Commands::run("deal no")}, 'ไม่แลกเปลี่ยน');
	$self->addMenu($dealMenu, 'แสดงรายการแลกเปลี่ยน', sub {Commands::run("dl")}, 'แสดงรายการแลกเปลี่ยน');

	#shop
	my $shopMenu = new Wx::Menu();
	$menu->Append($shopMenu, 'ร้านค้า');
	$self->addMenu($shopMenu, 'โหลด shop.txt', sub {Commands::run("reload shop.txt")}, 'โหลด shop.txt');
	$self->addMenu($shopMenu, 'เปิดร้าน', sub {Commands::run("openshop")}, 'เปิดร้าน');
	$self->addMenu($shopMenu, 'ปิดร้าน', sub {Commands::run("closeshop")}, 'ปิดร้าน');
	$self->addMenu($shopMenu, 'ดูสินค้าที่ขายได้', sub {Commands::run("al")}, 'ดูสินค้าคงเหลือ');
	$self->addMenu($shopMenu, 'ดูชื่อร้านค้ารอบๆ', sub {Commands::run("vl")}, 'ดูชื่อร้านค้ารอบๆ');
	$self->addMenu($shopMenu, 'ดูชื่อสินค้าที่จะขาย', sub {Commands::run("testshop")}, 'ดูชื่อสินค้าที่จะขาย');

	# party/guild/friend
	my $pgfMenu = new Wx::Menu();
	$menu->Append($pgfMenu, 'ตี้/กิว/เพื่อน');
	$self->{shareParty1}  = $self->addMenu($pgfMenu, 'แชตี้', sub {Commands::run("party share 1")}, 'แชตี้');
	$self->{shareParty0}  = $self->addMenu($pgfMenu, 'ปิดแชตี้', sub {Commands::run("party share 0")}, 'ปิดแชตี้');
	$pgfMenu->AppendSeparator;
	$self->addMenu($pgfMenu, 'เข้า์ติ้็', sub {Commands::run("party join 1")}, 'เข้าติ็');
	$self->addMenu($pgfMenu, 'ไม่เข้าตี้', sub {Commands::run("party join 0")}, 'ไม่เข้าตี้');
	$self->addMenu($pgfMenu, 'ออกจากตี้', sub {Commands::run("party leave")}, 'ออกจากตี้');
	$pgfMenu->AppendSeparator;
	$self->addMenu($pgfMenu, 'ยอมรับเป็นเพื่อน', sub {Commands::run("friend accept")}, 'ยอมรับเป็นเพื่อน');
	$self->addMenu($pgfMenu, 'ไม่รับเป็นเพื่อน', sub {Commands::run("friend reject")}, 'ไม่รับเป็นเพื่อน');
	$pgfMenu->AppendSeparator;
	$self->addMenu($pgfMenu, 'เข้ากิล', sub {Commands::run("guild join 1")}, 'เข้ากิล');
	$self->addMenu($pgfMenu, 'ไม่เข้ากิว', sub {Commands::run("guild join 0")}, 'ไม่เข้ากิว');
	$pgfMenu->AppendSeparator;
	$self->addMenu($pgfMenu, 'ดูสมาชิกในตี้', sub {Commands::run("party")}, 'ดูสมาชิกในตี้');
	$self->addMenu($pgfMenu, 'ดูสมาชิกในกิว', sub {Commands::run("guild member")}, 'ดูสมาชิกในกิว');
	$self->addMenu($pgfMenu, 'ดูขัอมูลกิว', sub {Commands::run("guild info")}, 'ดูขัอมูลกิว');
	$self->addMenu($pgfMenu, 'ดูรายชื่อเพื่น', sub {Commands::run("friend")}, 'ดูรายชื่อเพื่น');

	
	# Menu Character Slot
	my $charMenu = new Wx::Menu();
	$menu->Append($charMenu, 'เลือกตัวละคร');
	$self->{char0} = $self->addMenu($charMenu, 'เลือกตัวละคร1', sub {Commands::run("conf char 0"); Commands::run("relog")}, ' Select Value 0');
	$self->{char1} = $self->addMenu($charMenu, 'เลือกตัวละคร2', sub {Commands::run("conf char 1"); Commands::run("relog")}, ' Select Value 1');
	$self->{char2} = $self->addMenu($charMenu, 'เลือกตัวละคร3', sub {Commands::run("conf char 2"); Commands::run("relog")}, ' Select Value 2');
	$charMenu->AppendSeparator;
	$self->{char3} = $self->addMenu($charMenu, 'เลือกตัวละคร4', sub {Commands::run("conf char 3"); Commands::run("relog")}, ' Select Value 3');
	$self->{char4} = $self->addMenu($charMenu, 'เลือกตัวละคร5', sub {Commands::run("conf char 4"); Commands::run("relog")}, ' Select Value 4');
	$self->{char5} = $self->addMenu($charMenu, 'เลือกตัวละคร6', sub {Commands::run("conf char 5"); Commands::run("relog")}, ' Select Value 5');
	$charMenu->AppendSeparator;
	$self->{char6} = $self->addMenu($charMenu, 'เลือกตัวละคร7', sub {Commands::run("conf char 6"); Commands::run("relog")}, ' Select Value 6');
	$self->{char7} = $self->addMenu($charMenu, 'เลือกตัวละคร8', sub {Commands::run("conf char 7"); Commands::run("relog")}, ' Select Value 7');
	$self->{char8} = $self->addMenu($charMenu, 'เลือกตัวละคร9', sub {Commands::run("conf char 8"); Commands::run("relog")}, ' Select Value 8');

	# Help menu
	my $helpMenu = new Wx::Menu();
	$self->addMenu($helpMenu, T('&คู่มือ Openkore') . "\tF1", \&onManual, T('Read the manual'));
	$self->addMenu($helpMenu, T('&Openkore กระดานสนทนา') . "\tShift-F1", \&onForum, T('Visit the forum'));
	$self->createHelpMenu($helpMenu) if ($self->can('createHelpMenu'));
	$menu->Append($helpMenu, T('&ช่วยเหลือ'));
...
# 	}
	...
	$self->{mBooleanSetting}{'wx_npcTalk'} = $self->addCheckMenu (
		$parentMenu, T('ใช้ Wx NPC Talk'), sub { $self->onBooleanSetting ('wx_npcTalk'); },
		T('Open a dialog when talking with NPCs')
	);
	
	$self->{mBooleanSetting}{'wx_captcha'} = $self->addCheckMenu (
		$parentMenu, T('ใช้ Wx captcha'), sub { $self->onBooleanSetting ('wx_captcha'); },
		T('Open a dialog when receiving a captcha')
	);
	
	$self->{mBooleanSetting}{'wx_map_route'} = $self->addCheckMenu (
		$parentMenu, T('โชว์เส้นทางการเดินของบอท'), sub { $self->onBooleanSetting ('wx_map_route'); },
